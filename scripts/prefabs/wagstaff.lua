...
local common_postinit = function(inst) 
    ...
    inst:AddTag("weakstomach")
        ...
        -- BUG FIX (v2.0.3): XP reset signal net_bool.
        -- Previously, modmain.lua created this net_bool inside AddPrefabPostInit("world"),
        -- but net_bools created in PostInit do NOT replicate their values from server to
        -- client (the engine only replicates net_vars declared during the prefab fn /
        -- common_postinit / master_postinit phase). This caused the server's :set(true)
        -- to never reach the client — the client always read :value() == false.
        -- By declaring it here in common_postinit (same place as inst.spy, which is a
        -- known-working net_bool), it is registered with the engine's network
        -- serialization and WILL replicate. modmain.lua sets it on the server and reads
        -- it on the client to trigger the XP/skill reset on fresh worlds.
        inst.wagstaff_needs_xp_reset = net_bool(inst.GUID, "wagstaff_needs_xp_reset")
